...
if (container.children.length >= 2) {
  container.removeChild(container.lastElementChild); // remove <p>
  container.removeChild(container.lastElementChild); // remove <h1>
}
...
function updateTaskCount() {
  const count = document.querySelectorAll(".newtask-p").length;
  const countDisplay = document.querySelector(".taskes-p"); // A separate <p> for count
